...
int XModelMesh::CheckBasicCollision(Float3 &ref, Float2 &move_vector, int view_rotation, Int3 &viewport)
{
	...
	int absX = static_cast<int>(EndX - StartX);
	int absY = static_cast<int>(EndY - StartY);
		...
		for (int z = beginZ + c; z < beginZ + 3 + d; ++z)
		{
			...
			for (int i = 0; i < c.size; ++i)
			{
				...
				if (viewport._1 == -1)
				{
					...
					int dx = static_cast<int>(collider.x_offset - StartX);
					int dy = static_cast<int>(collider.z_offset - StartY);
...
void XModelMesh::LoadObjectDefintions()
{
	...
	object_buffer[++buffer_index] = { -0.5F, -0.1f, 0.5F, 1.0f, 1.0f };
	object_buffer[++buffer_index] = { -0.5F, 0.1f, 0.5F, 0.0f, 1.0f };
	object_buffer[++buffer_index] = { -0.5F, 0.1f, -0.5F, 0.0f, 0.0f };

	object_buffer[++buffer_index] = { -0.5F, 0.1f, -0.5F, 0.0f, 0.0f };
	object_buffer[++buffer_index] = { -0.5F, -0.1f, -0.5F, 1.0f, 0.0f };
	object_buffer[++buffer_index] = { -0.5F, -0.1f, 0.5F, 1.0f, 1.0f };
...
*/
int32_t XModelMesh::CreateTexturedSquare(Vertex32Byte *verts, int offset, Float3 Col, int texture_width, int texture_height, int drawX, int drawY)
{
	float offsetX = (2 * drawX) / (float)ScreenManagerSystem::GetScreenWidth();
	float offsetY = (2 * drawY) / (float)ScreenManagerSystem::GetScreenHeight();

	float width = (2 * texture_width) / (float)ScreenManagerSystem::GetScreenWidth();
	float height = (2 * texture_height) / (float)ScreenManagerSystem::GetScreenHeight();
